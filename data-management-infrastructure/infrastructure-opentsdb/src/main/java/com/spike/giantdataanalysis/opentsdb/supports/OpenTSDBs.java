...
public class OpenTSDBs {
    ...
    public static void SYNC_ADDPOINT(TSDB tsdb, String metric, long timestamp, Object value, Map<String, String> tags) {
    ...
    public static <T> void SYNC_ADDPOINT(TSDB tsdb, List<AbstractMetricPoint> points) {
        ...
        for (AbstractMetricPoint point : points) {
            ...
            if (point instanceof LongMetricPoint) {
                deferred = tsdb.addPoint(point.getMetric(), point.getTimestamp(), ((LongMetricPoint) point).getValue(), point.getTags());
            } else if (point instanceof FloatMetricPoint) {
                deferred = tsdb.addPoint(point.getMetric(), point.getTimestamp(), ((FloatMetricPoint) point).getValue(), point.getTags());
            } else if (point instanceof DoubleMetricPoint) {
                deferred = tsdb.addPoint(point.getMetric(), point.getTimestamp(), ((DoubleMetricPoint) point).getValue(), point.getTags());
            } else {
                LOG.error("Invalid value in metric point");
                continue;
            }
    ...
    public static void TSV() {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader("src/main/resources/daily-minimum-temperatures-in-me.tsv"));) {
