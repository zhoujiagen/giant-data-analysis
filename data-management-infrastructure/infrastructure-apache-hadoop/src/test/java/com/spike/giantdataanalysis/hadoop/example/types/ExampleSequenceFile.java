package com.spike.giantdataanalysis.hadoop.example.types;
 ...
 * @author zhoujiagen
 */
public class ExampleSequenceFile {
  ...
  public static void main(String[] args) throws IOException {
    String pathUri = ExampleConstants.DATA_TEST_OUTPUT_PATH + "/seqfile";
    if (args.length > 0 && StringUtils.isNotBlank(args[0])) {
      pathUri = args[0];
    }

    Configuration conf = new Configuration();
    Path path = new Path(ExampleConstants.DATA_TEST_OUTPUT_PATH);
    Hadoops.deleteLocalPath(conf, path, true);
  ...
  static void sort(Configuration conf, Path path) throws IOException {
    ...
    Hadoops.outputWritableSequenceFile(conf, output);
  }
}
