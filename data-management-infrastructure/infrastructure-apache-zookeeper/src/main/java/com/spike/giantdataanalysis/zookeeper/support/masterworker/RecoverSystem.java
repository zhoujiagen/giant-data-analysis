...
public class RecoverSystem {
  ...
  private void getAssignedWorkers() {
    zk.getChildren(//
      ZooKeepers.ASSIGN_PARENT_ZNODE_PATH, //
      false, //
    ...
    public void processResult(int rc, String path, Object ctx, List<String> children) {
      ...
      case OK:
        ...
        for (String worker : assignedWorkers) {
          getWorkerAssignments(ZooKeepers.ASSIGN_PARENT_ZNODE_PATH_PREFIX + worker);
        }
  ...
  private ChildrenCallback getWorkerAssignmentsCallback = new ChildrenCallback() {
    ...
    public void processResult(int rc, String path, Object ctx, List<String> children) {
      ...
      case OK:
        String worker = path.replace(ZooKeepers.ASSIGN_PARENT_ZNODE_PATH_PREFIX, "");
  ...
  private void recreateTask(RecreateTaskCtx ctx) {
    zk.create(//
      ZooKeepers.TASK_ZNODE_PATH_PREFIX + ctx.task,//
      ctx.data,//
  ...
  private void getStatuses() {
    zk.getChildren(//
      ZooKeepers.STATUS_PARENT_ZNODE_PATH, //
      false, //
      statusCallback, //
      null//
    );
  ...
  private void processAssignments() {
    ...
    for (String s : assignments) {
      ...
      deleteAssignment(ZooKeepers.TASK_PARENT_ZNODE_PATH_PREFIX + s);
      tasks.remove(s);
    }
