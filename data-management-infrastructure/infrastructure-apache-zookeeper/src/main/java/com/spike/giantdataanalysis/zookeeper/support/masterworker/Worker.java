...
public class Worker {
  ...
  public static void main(String[] args) throws InterruptedException, IOException {
    Worker worker = new Worker(ZooKeepers.DEFAULT_QUORUM_CONN_STRING);
  ...
  private void createAssignZnode() {
    zk.create(//
      ZooKeepers.ASSIGN_WORKER_ZNONE_PATH_PREFIX + serverId,//
      new byte[0],//
  ...
  private void register() {
    ...
    zk.create(//
      ZooKeepers.WORKER_PARENT_ZNODE_PATH_PREFIX + workerName,//
      "Idle".getBytes(),//
      ZooKeepers.DEFAULT_ACLS,//
  ...
  private void getTasks() {
    zk.getChildren(//
      ZooKeepers.ASSIGN_PARENT_ZNODE_PATH_PREFIX + workerName,//
      newTaskWatcher, //
      getTaskCallback,//
      null//
    );
  ...
  private Watcher newTaskWatcher = new Watcher() {
    ...
    public void process(WatchedEvent event) {
      ...
      if (EventType.NodeChildrenChanged.equals(event.getType())) {
        ...
        if ((ZooKeepers.ASSIGN_PARENT_ZNODE_PATH_PREFIX + workerName).equals(event.getPath())) {
          getTasks();
        }
      }
    }
          ...
          executor.execute(new Runnable() {
            ...
            public void run() {
              ...
              for (String task : children) {
                zk.getData(//
                  ZooKeepers.ASSIGN_WORKER_ZNONE_PATH_PREFIX + serverId + "/" + task,//
                  false, //
                  cb, //
                  task//
                );
      ...
      case OK:
        ...
        executor.execute(new Runnable() {
          ...
          public void run() {
            ...
            zk.create(//
              ZooKeepers.STATUS_PARENT_ZNODE_PATH_PREFIX + (String) ctx, //
              "done".getBytes(), //
              ZooKeepers.DEFAULT_ACLS,//
              CreateMode.PERSISTENT, //
            ...
            zk.delete(//
              ZooKeepers.ASSIGN_WORKER_ZNONE_PATH_PREFIX + serverId + "/" + (String) ctx,//
              -1,//
              taskVoidCallback, //
              null//
            );
