...
public class Config {
  ...
  protected final HashMap<String, String> properties = 
    new HashMap<String, String>(); // MARK(zhoujiagen) 实际配置属性以这个为准
  ...
  */  
  ...
  protected void loadStaticVariables() {
    ...
    // MARK(zhoujiagen) Ganglia配置
    ganglia_host = this.getString("tsd.extension.ganglia.host");
    ganglia_port = this.getInt("tsd.extension.ganglia.port");
    
    // MARK(zhoujiagen) Kafka客户端配置
    kafkaPropertiesFile = this.getString("tsd.extension.plugin.kakfa.file");
    kafkaProperties = new Properties();
    try (Reader reader = new FileReader(kafkaPropertiesFile);) {
      kafkaProperties.load(reader);
    } catch (FileNotFoundException e) {
      LOG.error("load kafka properties file failed", e);
    } catch (IOException e) {
      LOG.error("load kafka properties file failed", e);
    }
  }
