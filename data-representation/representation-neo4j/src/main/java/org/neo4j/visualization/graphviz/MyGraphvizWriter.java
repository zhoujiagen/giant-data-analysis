/*
 * Copyright (c) 2002-2017 "Neo Technology," Network Engine for Objects in Lund AB
 * [http://neotechnology.com] This file is part of Neo4j. Neo4j is free software: you can
...
public class MyGraphvizWriter {
   ...
   * @param configuration the style parameters determining how the style of the output of this
   *          writer.
   */
  public MyGraphvizWriter(StyleParameter... configuration) {
    this(new GraphStyle(configuration));
  }

  public MyGraphvizWriter(GraphStyle style) {
    this.style = style;
  }
  ...
  public void emit(File dest, Walker walker) throws IOException {
    ...
    // stream.write(("digraph " + dest.getName() + " {").getBytes()); // DIFF
    emit(stream, walker);
    // stream.write("}".getBytes()); // DIFF
    stream.close();
  }
  ...
  public void emit(OutputStream outputStream, Walker walker) throws IOException {
    if (outputStream instanceof PrintStream) {
      ...
      emit(walker, new GraphvizRenderer(style, new PrintStream(outputStream, true,
