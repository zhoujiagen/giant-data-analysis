...
import com.spike.giantdataanalysis.procedure.mw.RecoverSystem.RecoverCallback;
...
public class Master {
  ...
  public static void main(String[] args) throws InterruptedException, IOException {
    String hostAndPort = MasterWorkerConstants.DEFAULT_QUORUM_CONN_STRING;
    ...
    master.createMetadataZnode(MasterWorkerConstants.WORKER_PARENT_ZNODE_PATH, new byte[0]);
    master.createMetadataZnode(MasterWorkerConstants.ASSIGN_PARENT_ZNODE_PATH, new byte[0]);
    master.createMetadataZnode(MasterWorkerConstants.TASK_PARENT_ZNODE_PATH, new byte[0]);
    master.createMetadataZnode(MasterWorkerConstants.STATUS_PARENT_ZNODE_PATH, new byte[0]);
  ...
  public void runForMaster() {
    ...
    zk.create(//
      MasterWorkerConstants.MASTER_ZNODE_PATH, //
  ...
  private Watcher masterExistsWatcher = new Watcher() {
    ...
    public void process(WatchedEvent event) {
      if (Event.EventType.NodeDeleted.equals(event.getType())) {
        ...
        if (event.getPath().equals(MasterWorkerConstants.MASTER_ZNODE_PATH)) {
  ...
  private Watcher workersChangeWatcher = new Watcher() {
    public void process(WatchedEvent event) {
      if (EventType.NodeChildrenChanged.equals(event.getType())) {
        ...
        if (MasterWorkerConstants.WORKER_PARENT_ZNODE_PATH.equals(event.getPath())) {
          getWorkers();
        }
      }
    }
  ...
  private void getAbsentWorkerTasks(String worker) {
    zk.getChildren(//
      MasterWorkerConstants.ASSIGN_PARENT_ZNODE_PATH_PREFIX + worker,//
      false, //
  ...
  private void recreateTask(RecreateTaskCtx ctx) {
    zk.create(//
      MasterWorkerConstants.TASK_PARENT_ZNODE_PATH_PREFIX + ctx.task, //
      ctx.data, //
      MasterWorkerConstants.DEFAULT_ACLS, //
      CreateMode.PERSISTENT,//
  ...
  private Watcher tasksChangeWatcher = new Watcher() {
    ...
    public void process(WatchedEvent event) {
      if (EventType.NodeChildrenChanged.equals(event.getType())) {
        ...
        if (MasterWorkerConstants.TASK_PARENT_ZNODE_PATH.equals(event.getPath())) {
  ...
  private void getTaskData(String task) {
    zk.getData(//
      MasterWorkerConstants.TASK_PARENT_ZNODE_PATH_PREFIX + task, //
      false, //
      taskDataCallback, //
      task//
    );
    ...
    public void processResult(int rc, String path, Object ctx, byte[] data, Stat stat) {
      ...
      case OK:
        ...
        String assignmentPath =
            MasterWorkerConstants.ASSIGN_PARENT_ZNODE_PATH_PREFIX + worker + "/" + (String) ctx;
  ...
  private void deleteTask(String name) {
    zk.delete(//
      MasterWorkerConstants.TASK_PARENT_ZNODE_PATH_PREFIX + name,//
      -1, //
      taskDeleteCallback,//
      null//
    );
